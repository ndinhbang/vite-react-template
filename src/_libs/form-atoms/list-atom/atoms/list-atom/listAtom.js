...
export function listAtom({ fields, ...config }) {
  ...
  const makeFormList = () => formBuilder(config.value).map(buildItem);
  ...
  const dirtyAtom = atom((get) => {
    const listUpdated = !arraysShallowEqual(get(_initialFormListAtom), get(_formListAtom));
