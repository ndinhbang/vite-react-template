export function setPath(target, paths, value) {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    if (i === paths.length - 1) {
      ...
      next = next[path] = current ?? (isNaN(paths[i + 1]) ? {} : []);
    }
  }
}
 ...
 */
export function formatDateString(date, type) {
...
function getIsoWeek(date) {
  ...
  // Start of the year
  const startUtcTime = new Date(Date.UTC(date.getUTCFullYear(), 0, 1)).getTime();
