...
const useCheckboxFieldProps = (field) => {
  const { value, ...props } = useFieldProps(field, (event) => event.currentTarget.checked);

  return { ...props, checked: value };
};

export const useRadio = ({ control, field }) => {
...
export const Radio = ({ field, control, children }) => children(useRadio({ control, field }));
